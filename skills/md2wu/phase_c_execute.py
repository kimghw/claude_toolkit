...
Writes:
  ...
  skill_md2wu/wu-<wu_key>__pre__meta.json
  skill_md2wu/corpus-<scope>__pre__manifest.json
  skill_md2wu/corpus-<scope>__pre__item_index.json
  skill_md2wu/corpus-<scope>__md2wu__issue_gate_report.json
"""
...
def main():
    ...
    # item_index triggers Phase B-0 skip; it must only appear after all artifacts
    ...
    for series_key, group_wus in wus_by_series.items():
        ...
        manifest_path = WORKROOT / f"corpus-{scope}__pre__manifest.json"
        manifest_tmp = manifest_path.with_suffix(".json.tmp")
        manifest_tmp.write_text(
            json.dumps(manifest, indent=2, ensure_ascii=False), encoding="utf-8")
        os.replace(str(manifest_tmp), str(manifest_path))

        # issue gate report (filter to this series)
        ...
        igr_path = WORKROOT / f"corpus-{scope}__md2wu__issue_gate_report.json"
        ...
        # item_index: item_id → [wu_keys] (list to support split WUs that map
        # one item to multiple wu_keys). Backward compat: reader accepts str|list.
        # Written LAST per scope, via atomic rename, so that skip detection never
        # sees a partial publishing state.
        ...
        idx_payload = {
            "corpus_scope": scope,
            "generated_at": now,
            "item_to_wu": item_to_wu,
        }
        idx_path = WORKROOT / f"corpus-{scope}__pre__item_index.json"
        idx_tmp = idx_path.with_suffix(".json.tmp")
        idx_tmp.write_text(
            json.dumps(idx_payload, indent=2, ensure_ascii=False), encoding="utf-8")
        os.replace(str(idx_tmp), str(idx_path))
