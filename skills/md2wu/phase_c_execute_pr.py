#!/usr/bin/env python3
"""Phase C for IACS PR — claim locks, chunk/WU packing, emit WU content + manifest.

Usage: phase_c_execute_pr.py <session_id>
...
def main():
    ...
    for group, group_series in merge_groups:
        ...
        wu_key = f"iacs_pr_merge_{hashlib.sha256(keys_str.encode()).hexdigest()[:8]}"
        constituent = []
        chunk_keys = []
        total = 0
        content_parts = []
    ...
    for series_key, group_wus in wus_by_series.items():
        # PR has a single logical series. Map series_key → scope cleanly:
        #   pr      → iacs_pr        (active)
        #   pr_del  → iacs_pr_del    (deleted/withdrawn)
        if series_key == "pr":
            scope = "iacs_pr"
        elif series_key == "pr_del":
            scope = "iacs_pr_del"
        else:
            scope = f"iacs_pr_{series_key}"
        manifest = {
            "corpus_scope": scope,
