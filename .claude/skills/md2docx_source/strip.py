...
**원본 source(.md) 는 절대 수정하지 않는다.** 치환 결과는 스킬 내부
`output/` 폴더에 `<원본_stem>_prep.md` 로 저장 (예:
`<skill_dir>/output/report_prep.md`).
...
def prep_path_for(md_path: Path) -> Path:
    """기본 산출물 경로 — skill 내부 output/<stem>_prep.md.
    이미 `_prep` 로 끝나는 stem 이면 suffix 를 중복으로 붙이지 않는다."""
    stem = md_path.stem if md_path.stem.endswith(PREP_SUFFIX) else md_path.stem + PREP_SUFFIX
    return OUTPUT_DIR / f"{stem}{md_path.suffix}"
