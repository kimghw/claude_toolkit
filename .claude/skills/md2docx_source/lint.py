#!/usr/bin/env python3
"""
md2docx/lint.py — source(.md) 검사용. Pandoc convert 전 Markdown 넘버링/헤딩 사전 검토

흐름 위치: source(.md) 전처리 단계 = lint + strip 중 lint 담당.

목적
====
회사 양식 docx 로 convert 하기 전, source(.md) 의 ordered list 넘버링과
...
Usage
=====
    python lint.py <source.md>
    python lint.py <source.md> --json
    python lint.py <source.md> --tool builtin   # 외부 도구 무시, 내장만 사용
...
def main():
    ap = argparse.ArgumentParser(description="source(.md) 넘버링/헤딩 사전 검토")
    ap.add_argument("md", help="source markdown 파일")
