#!/usr/bin/env python3
"""
md2docx/postprocess_tables.py — pandoc convert 후 output docx 의 표 디자인 적용

주의: 이 모듈의 식별자 `target_style_id`, `target_styles_xml`,
`adjust_tblpr_for_target` 등에 등장하는 'target' 은 canonical target
(회사 양식 raw docx) 이 아니라 **output (=후처리 대상 docx) 의 styleId**
의미다. 식별자명은 외부 인터페이스로 보존하고, 사용자에게 보이는 어휘만
canonical 로 정리한다.

문제:
    pandoc 출력 output docx 의 표는 `<w:tblStyle w:val="Table"/>` 를 참조하고
    `Table` 스타일은 회사 `Table Grid` (aa) 를 basedOn 한다. 그러나 Word 는
    table 스타일의 `tblBorders` 와 `tblStylePr` (firstRow/firstCol conditional)
    를 basedOn 만으로 완전히 propagate 하지 않는다. 결과적으로 output 의
    표는 가는 회색 테두리 + conditional 채움 비활성 상태가 된다.

해결:
    reference 의 Table Grid 스타일 정의 전체를 output 의 `Table` 스타일에
...
Usage:
    python postprocess_tables.py <output.docx>                          # reference 자동 탐색
    python postprocess_tables.py <output.docx> --reference <ref.docx>   # 명시
    python postprocess_tables.py <output.docx> --no-style-clone         # 스타일 클론 생략
    python postprocess_tables.py <output.docx> --out <new.docx>
    python postprocess_tables.py <output.docx> --dry-run                # 표 개수만
...
# reference 표의 tblLook — firstRow + firstColumn + noVBand
REF_TBLLOOK = (
...
def adjust_tblpr_for_target(ref_tblpr_inner: str, target_style_id: str) -> str:
    ...
    (여기서 target_style_id 는 output(=후처리 대상 docx) 의 styleId 의미 — canonical target 아님)
    """
    return re.sub(
...
def extract_style_paragraph_jc(ref_styles_xml: str, style_name: str = "Table Grid"):
    ...
    reference 가 Table Grid 스타일에 `<w:pPr><w:jc w:val="center"/></w:pPr>` 같은
    기본 정렬을 박아둔 경우 그것을 output docx 의 표 셀 단락에 반영하기 위함.
...
# ---------------------------------------------------------------------------
# 스타일 클론: reference 의 Table Grid → output docx 의 Table 스타일
# ---------------------------------------------------------------------------
...
def clone_table_style(ref_styles_xml: str, target_styles_xml: str,
    ...
    """reference 의 <ref_style_name> 스타일 정의를 output 의 <target_style_id> 스타일에
    전체 복사. 자기 자신 name 은 target_style_id 로 유지 (pandoc 매핑 보존).
    basedOn 은 제거해 self-contained 정의로 만든다.

    참고: 매개변수 target_styles_xml / target_style_id 의 'target' 은
    output (=후처리 대상 docx) 의 styles.xml / styleId 의미 — canonical target 아님.
...
def process_document_xml(doc_xml: str, ref_tblpr_inner: str = None,
    ...
    """output document.xml 의 모든 <w:tbl> 에 대해 patch 적용.
...
def postprocess(docx_in: Path, docx_out: Path,
    ...
    pandoc 이 jc 를 박지 않은 output 의 표 셀 단락에 그 값을 inline 으로 적용한다.
    """
...
def main():
    ap = argparse.ArgumentParser(description="pandoc convert 후 output docx 의 표 디자인 적용")
    ap.add_argument('docx', help="대상 output docx (in-place 또는 --out 지정)")
    ap.add_argument('--reference', help="reference docx (스타일 클론 소스)")
    ...
    ap.add_argument('--target-style-id', default="Table",
                    help="output docx 에서 덮어쓸 styleId (기본: 'Table' — pandoc 출력). "
                         "여기서 'target' 은 output styleId 의미, canonical target 아님.")
    ...
    # reference 결정: --reference 우선, 아니면 md2docx/template/ 의 가장 최근 캐시
    ref_path = None
    if not args.no_style_clone:
        if args.reference:
            ...
            md2docx_template = here.parent / "md2docx" / "template"
            if md2docx_template.exists():
                cached = [p for p in md2docx_template.glob("*.docx") if p.is_file()]
                if cached:
                    ref_path = max(cached, key=lambda p: p.stat().st_mtime)
