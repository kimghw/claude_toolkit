...
입력은 보통 md2docx 가 만든 pandoc 출력:
    cwd/md2docx_output/<stem>_pandoc.docx
출력은:
    cwd/md2docx_layout/<stem>_output.docx     (stem 의 '_pandoc' 접미사 자동 제거)
...
기본 출력 경로:
    cwd/md2docx_layout/<stem>_output.docx
...
예시:
    ...
    python md2docx_layout.py md2docx_output/report_pandoc.docx
    # → md2docx_layout/report_output.docx
    ...
    # 표 간격만 끄고 싶을 때
    python md2docx_layout.py md2docx_output/report_pandoc.docx --skip-table-spacing

    # 사용자 별도 settings.json 사용
    python md2docx_layout.py md2docx_output/report_pandoc.docx --settings my_settings.json
...
def derive_output_path(input_path: Path) -> Path:
    ...
    return Path.cwd() / "md2docx_layout" / f"{stem}_output.docx"
