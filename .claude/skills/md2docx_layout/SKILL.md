---
name: md2docx_layout
description: pandoc 으로 만든 output(.docx) 의 표·표간격·페이지·머리바닥 후처리 orchestrator. postprocess_tables / postprocess_table_spacing / postprocess_page / (옵션) postprocess_header_footer 를 md2docx_layout.py 가 subprocess 로 순차 호출. reference 미지정 시 md2docx/template/ 의 가장 최근 캐시 자동 사용. 기본 출력은 cwd/md2docx_layout/<stem>_output.docx (stem 의 '_pandoc' 접미사 자동 제거). 표 직후 단락 간격은 settings.json 에서 관리 (twips 단위, mode=ensure/insert/patch_existing/off). --header-source <src.docx> 지정 시 그 docx 의 텍스트 있는 가장 풍부한 header/footer 한 쌍을 default 로 박는다 (스타일·numbering 불변). 목록단락·표준 단락 pStyle ↔ target list 스타일 매핑(헤딩 제외)은 별도 md2docx_pstyle 스킬 사용.
---
...
- 입력: `<input.docx>` (보통 [`md2docx`](../md2docx/SKILL.md) 가 만든 `cwd/md2docx_output/<stem>_pandoc.docx`)
- 출력: `cwd/md2docx_layout/<stem>_output.docx` (기본). `_pandoc` 접미사가 있으면 자동 제거.
...
| `--out <path>` | output 경로 (기본 `cwd/md2docx_layout/<stem>_output.docx`) |
...
- **output**: `cwd/md2docx_layout/<stem>_output.docx`
...
```powershell
# 표준 흐름 — md2docx 가 만든 md2docx_output/<stem>_pandoc.docx 를 후처리
python .claude\skills\md2docx_layout\md2docx_layout.py md2docx_output\report_pandoc.docx
# → cwd\md2docx_layout\report_output.docx
...
```
[입력] <input.docx>  (보통 cwd/md2docx_output/<stem>_pandoc.docx)
   ↓
[reference 결정]
   ├─ --reference 시: 그 경로 사용
...
[4/N] postprocess_header_footer: output 에 in-place  (--header-source 시만)
   ...
   ↓
[출력] cwd/md2docx_layout/<stem>_output.docx
```
...
md2docx (source + ref → output, pandoc 호출)
   ↓ output (cwd/md2docx_output/<stem>_pandoc.docx)
md2docx_layout (본 스킬 — 표·페이지 후처리)
   ↓ output (cwd/md2docx_layout/<stem>_output.docx)
md2docx_pstyle (목록단락·표준 pStyle ↔ target list 스타일 매핑 — 별도 스킬)
   ↓ output (cwd/md2docx_layout/<stem>_output.docx, in-place patch)
```
...
| 출력 | `cwd/md2docx_output/<stem>_pandoc.docx` | `cwd/md2docx_layout/<stem>_output.docx` | `<stem>_output.docx` (in-place) |
...
→ 그대로 stem 에 `_output` 만 부착. 예: `report.docx` → `cwd/md2docx_layout/report_output.docx`.
...
→ md2docx 가 `cwd/md2docx_output/<stem>_pandoc.docx` 를 만들면, 이어서 `md2docx_layout cwd/md2docx_output/<stem>_pandoc.docx` 호출. reference 는 둘 다 `md2docx/template/` 최근 캐시를 fallback 으로 잡으므로 동일 reference 가 자동 적용된다.
