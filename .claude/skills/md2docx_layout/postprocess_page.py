#!/usr/bin/env python3
"""
md2docx/postprocess_page.py — pandoc convert 후 output docx 의 페이지 레이아웃 동기화

작업: reference 의 sectPr 을 output 에 덮어써 페이지 여백/용지 크기를 reference 와
      동일하게 보장한다.

문제:
    pandoc 은 보통 reference 의 sectPr 를 그대로 사용하지만, 사용자가
    reference 를 교체하거나 별도 sectPr 가 끼어들면 output 의 페이지 여백이
    어긋날 수 있다.

해결:
    reference docx 의 마지막 sectPr 에서 <w:pgSz>, <w:pgMar>, <w:cols>,
    <w:docGrid> 를 추출해 output docx 의 모든 sectPr 에 덮어쓴다. 이로써 회사
    양식의 페이지 여백/용지 크기를 output 에 보장한다.

Usage:
    python postprocess_page.py <output.docx>                          # reference 자동 탐색
    python postprocess_page.py <output.docx> --reference <ref.docx>   # 명시
    python postprocess_page.py <output.docx> --out <new.docx>
    python postprocess_page.py <output.docx> --dry-run                # 추출 정보만
...
def apply_page_settings(doc_xml: str, settings: dict) -> tuple:
    """output 의 모든 <w:sectPr> 의 pgSz/pgMar/cols/docGrid 요소를 settings 의 값으로 교체.
...
def postprocess(docx_in: Path, docx_out: Path,
    ...
    reference 의 pgSz/pgMar/cols/docGrid 를 output 의 모든 sectPr 에 덮어써
    페이지 여백을 reference 와 동일하게 보장.
    """
    # reference 가 없으면 스킵
...
def main():
    ap = argparse.ArgumentParser(description="pandoc convert 후 output docx 의 페이지 레이아웃을 reference 와 동기화")
    ap.add_argument('docx', help="대상 output docx (in-place 또는 --out 지정)")
    ap.add_argument('--reference', help="reference docx (페이지 설정 소스)")
