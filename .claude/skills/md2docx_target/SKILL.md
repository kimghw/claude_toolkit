---
name: md2docx_target
description: 사용자 target.docx (회사 양식 raw) → <target_stem>_reference.docx (Pandoc 호환 reference) 변환만 수행하는 단일 기능 스킬. md→docx 변환(convert) 은 포함하지 않음. 회사 어휘(heading 1, Quote 등) 와 Pandoc 어휘(Heading 1, Block Text 등) 의 불일치를 SEMANTIC_HINTS / STUB_DEFINITIONS 기반 basedOn 상속으로 자동 해결해, <cwd>/md2docx_target/ 폴더에 <target_stem>_reference.docx 와 <target_stem>_reference.report.md 를 저장한다. 회사 numbering 은 markdown 리스트에 자동 적용하지 않으며(Pandoc 기본 유지) AskUserQuestion 게이트도 없다.
---
...
- 출력: `<cwd>/md2docx_target/<target_stem>_reference.docx` (Pandoc `--reference-doc` 입력으로 쓸 수 있는 docx) + `<cwd>/md2docx_target/<target_stem>_reference.report.md` (매핑 분석 리포트)
...
| **reference** | pandoc `--reference-doc` 가 받는 스타일 docx. target 을 매핑/정규화한 결과. 본 스킬의 산출물 |
| **map** | target → reference 변환 단계. 본 스킬이 수행하는 유일한 작업 |

## 인자 형식

```
python md2docx_target.py <target.docx>                           # 기본 — <cwd>/md2docx_target/ 에 저장
python md2docx_target.py <target.docx> --out <path.docx>         # 출력 경로 명시
python md2docx_target.py <target.docx> --map <mapping.json>      # 사용자 매핑 오버라이드
python md2docx_target.py <target.docx> --report <path.md>        # 리포트 경로 명시
```

## 산출물

target 1개당 reference docx + 리포트 1쌍이 생성된다:

| 산출물 | 경로 패턴 | 내용 |
|---|---|---|
| reference docx | `<cwd>/md2docx_target/<target_stem>_reference.docx` | target 의 styles.xml 에 Pandoc 어휘 스타일을 `basedOn` 으로 추가한 결과. Pandoc `--reference-doc` 입력으로 바로 사용 |
| 리포트 | `<cwd>/md2docx_target/<target_stem>_reference.report.md` | 매핑 plan 분석 (exact / case_mismatch / semantic / stub / missing) + target 의 numbering.xml 참고용 덤프 |

stdout 신호 (rc=0 시) 와 에러 형식 상세는 §작동 흐름 의 "신호" 절 참조.

## 산출물 명명

**target 파일명 stem 뒤에 `_reference` 접미사** (분리자는 언더스코어 `_`):

| target | reference docx | report |
|---|---|---|
| `mydoc.docx` | `<cwd>/md2docx_target/mydoc_reference.docx` | `<cwd>/md2docx_target/mydoc_reference.report.md` |
| `회사양식.docx` | `<cwd>/md2docx_target/회사양식_reference.docx` | `<cwd>/md2docx_target/회사양식_reference.report.md` |
| `408_Statement.docx` | `<cwd>/md2docx_target/408_Statement_reference.docx` | `<cwd>/md2docx_target/408_Statement_reference.report.md` |

리포트는 reference docx 의 stem 에 `.report.md` 를 붙인다 (`<target_stem>_reference.report.md`). 같은 target 으로 재호출 시 덮어쓰기.
...
| 도구 | 용도 |
|:---|:---|
| `Bash` | `python .claude/skills/md2docx_target/md2docx_target.py <target.docx>` 실행 |
| `Read` | 산출물 `<cwd>/md2docx_target/<stem>_reference.report.md` 검토 |
...
```powershell
# 기본 — <cwd>/md2docx_target/ 에 reference 저장
python .claude\skills\md2docx_target\md2docx_target.py company.docx
# → <cwd>\md2docx_target\company_reference.docx
# → <cwd>\md2docx_target\company_reference.report.md

# 출력 경로 명시
python .claude\skills\md2docx_target\md2docx_target.py company.docx --out company_reference.docx

# 사용자 매핑 오버라이드 — 특정 Pandoc 이름을 target 의 특정 스타일에 강제 매핑
python .claude\skills\md2docx_target\md2docx_target.py company.docx --map mapping.json
```
...
본 스킬이 reference 에 추가(또는 매칭 확인)하는 Pandoc 스타일 전체 목록 (`md2docx_target.py` 의 `PANDOC_STYLES` 상수). severity 는 누락 영향도 — `critical` 은 시각적으로 깨지고, `important` 는 일부 요소 무서식, `optional` 은 편의용.
...
전체 매핑 후보·stub 디폴트값은 [`decisions.md`](./decisions.md) §3·§4, 코드는 `md2docx_target.py` 의 `SEMANTIC_HINTS` / `STUB_DEFINITIONS`.
...
md2docx_target.py 호출
   ...
   ↓
[출력] <cwd>/md2docx_target/<target_stem>_reference.docx
...
- [`md2docx_target.py`](./md2docx_target.py) — map 분석·적용 엔진 (단일 진입점). target → reference + report 생성
- [`decisions.md`](./decisions.md) — 자동 매핑 결정 규칙
- 산출물 폴더: `<cwd>/md2docx_target/` — 호출 시 자동 생성, 같은 target 으로 재호출 시 1쌍 덮어쓰기
