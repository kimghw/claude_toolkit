#!/usr/bin/env python3
"""
md2docx_pstyle — output(.docx) 의 줄 단위 스타일을 target.docx 어휘 기준으로 정규화
...
서브커맨드:
    scan   <output.docx> --target <target.docx> [--out-report <json>]
        → scan.py 호출. JSON 보고서 생성.
    apply  <output.docx> <decisions.json> [--out <patched.docx>]
...
==========
  ...
  md2docx_pstyle scan <output.docx> --target <target.docx> [--out-report <json>]
      target 의 heading_inventory / list_styles / marker_hierarchy 와
      output 의 pstyle_usage 를 추출해 JSON 보고서 생성.
      기본 출력: cwd/output/<output_stem>_line.json   (단일 파일 — 재스캔 시 덮어쓰기)

  md2docx_pstyle apply <output.docx> <decisions.json> [--out <patched.docx>]
      decisions.json 의 매핑대로 output 의 pStyle / ind / numPr 정규화.
      기본: in-place. --out 지정 시 별도 파일.

작동 흐름
========

  [1] scan        → output/<output_stem>_line.json
