...
**output 의 `표준` / `목록단락(List Paragraph)` 단락의 pStyle 을 target 의 list 스타일로 매핑.**
...
- **output 측 대상**: output 의 `standard` 그룹 (Normal/Compact/표준/Body Text 등) + `list_styled` 그룹 (List Paragraph 계열 pStyle, numPr 없음).
...
```
[md2docx]        source.md + reference → pandoc/<stem>_pandoc.docx
[md2docx_layout]   pandoc/...           → output/<stem>_output.docx   (표·페이지만)
[md2docx_pstyle]   output/<stem>_output.docx + target.docx → in-place 정규화
                 (헤딩 pStyle + 리스트 pStyle/numPr + 마커 ind)
```
...
| output 그룹 kind | 매핑 후보 | 결과 action |
|---|---|---|
...
- `heading` — output 의 `Heading 1` ~ `Heading N`. target heading inventory 가 있어 매핑 가능은 하지만 본 스킬의 주 책임은 아님 (보통 pandoc 이 이미 올바른 Heading style 을 박는다). 보조로 `rename` 가능.
...
| 용어 | 의미 |
|---|---|
| **target** | 회사 양식 raw docx — heading/list 스타일·마커 hierarchy 의 source of truth |
| **output** | 정규화 대상 docx (보통 `md2docx_layout` 가 만든 output, 없으면 pandoc 직출력) |
| **heading_inventory** | target.styles.xml 에서 추출한 heading 계열 스타일 (id, name, basedOn) |
| **list_styles** | target.styles.xml 에서 추출한 List Paragraph 계열 스타일 (목록단락, List Paragraph 등) |
| **standard_styles** | target.styles.xml 에서 추출한 표준/본문 계열 스타일 (Normal, Standard, 표준, Compact, Body Text 등) |
| **marker_hierarchy** | target.document.xml 의 마커 단락에서 학습한 (level, marker, ind, 공백) 리스트 |
| **pstyle_usage** | output.document.xml 에서 단락별 (idx, pStyle, has_numPr, marker, ind) 인벤토리 |
...
| `md2docx_pstyle scan <output.docx> --target <target.docx>` | scan.py 만 호출 (decisions.json 생성용) |
| `md2docx_pstyle apply <output.docx> <decisions.json>` | apply.py 만 호출 |
| `md2docx_pstyle scan <output.docx> --target <target.docx> --out-report <json>` | 보고서 경로 지정 |
| `md2docx_pstyle apply <output.docx> <decisions.json> --out <patched.docx>` | 별도 출력 경로 |
...
```
[1] scan.py  output=<output.docx>  target=<target.docx>  --out-report <json>
        ├ target → heading_inventory[]  (heading 계열 스타일 메타)
        ├ target → list_styles[]        (List Paragraph 계열 스타일 메타)
        ├ target → standard_styles[]    (Normal/Standard/표준/본문 계열 스타일 메타)
        ├ target → marker_hierarchy[]   (마커 + 공백 + ind, level 부여)
        ├ output → output_style_names   (styleId → name 매핑, kind 분류용)
        ├ output → pstyle_usage[]       (단락 enumerate, pStyle/numPr/marker/ind)
        └ output/<output_stem>_line.json   (단일 파일 — 재스캔 시 덮어쓰기)
...
[3] apply.py  <output.docx>  <decisions.json>  [--out patched.docx]
...
```json
{
  ...
  "output": "output/report_output.docx",
  "heading_inventory": [
...
| 단계 | 파일명 패턴 | 위치 | 정책 |
|---|---|---|---|
| scan 결과 | `<output_stem>_line.json` | `cwd/output/` | 같은 output 재스캔 시 덮어쓰기 |
| 사용자 결정 (decisions) | `<output_stem>_line_decisions.json` | `cwd/output/` | 사용자/Claude 가 작성 |
| apply 결과 | `<output_stem>.docx` | in-place 또는 `--out` | 기본은 input 을 덮어쓰기 |

예) `report_output.docx` 입력 →
- `output/report_output_line.json` (scan)
- `output/report_output_line_decisions.json` (사용자 결정)
- `output/report_output.docx` (apply 후 — in-place)
...
python .claude\skills\md2docx_pstyle\md2docx_pstyle.py scan ^
    output\report_output.docx --target targets\company.docx

# [2] Claude 가 출력된 report 를 읽고 사용자에게 AskUserQuestion 으로 그룹별 매핑 받음
#     → output\report_output_line_decisions.json 작성

# [3] apply
python .claude\skills\md2docx_pstyle\md2docx_pstyle.py apply ^
    output\report_output.docx output\report_output_line_decisions.json
```

직접 호출도 가능:

```powershell
python .claude\skills\md2docx_pstyle\scan.py   output\report.docx --target target.docx
python .claude\skills\md2docx_pstyle\apply.py  output\report.docx decisions.json --out final.docx
```
...
**Q. output 에 헤딩이 6단계인데 target 에는 4단계만 있으면?**
