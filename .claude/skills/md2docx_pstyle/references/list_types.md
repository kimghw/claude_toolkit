...
`md2docx_pstyle/scan.py` 가 input docx 의 list 단락(`<w:numPr>` 보유)을 묶을 때 쓰는 분류 키와, AskUserQuestion 에서 사용자에게 제시할 마커 후보의 카탈로그.
...
- input 의 ilvl 그대로 사용. md 공백 lookup 불필요.
...
- target marker_hierarchy 가 input ilvl 보다 얕으면 → **Claude 가 decisions.json 작성 시 직접 매핑.** 보통 가장 깊은 level 의 marker 를 fallback 으로 쓰지만, scan.py 는 자동 처리하지 않음 (위 §3-B 의 자동화 범위 노트 참조)
