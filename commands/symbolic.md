...
원본 레포: `/home/kimghw/claude_toolkit/` (구조: `agents/`, `commands/`, `skills/`)
대상 디렉토리: `<프로젝트 루트>/.claude/`

## 동작 규칙

1. **사전 점검**
   - `ls /home/kimghw/claude_toolkit/`로 원본 존재·구조 확인.
...
3. **링크 생성 규칙**
   - 절대 경로 심볼릭 링크 사용: `ln -s /home/kimghw/claude_toolkit/<rel> <프로젝트 루트>/.claude/<rel>`
...
- `/symbolic all` → `.claude/agents`, `.claude/commands`, `.claude/skills` 3개 디렉토리 링크
- `/symbolic skills/pdf2md skills/md2wu` → `.claude/skills/` 내부에 2개 스킬 개별 링크
- `/symbolic` (인자 없음) → 원본 구조 제시 후 사용자에게 선택을 물음
